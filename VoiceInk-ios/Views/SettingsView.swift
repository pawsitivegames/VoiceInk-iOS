            ...
            Section(header: Text("Language Settings")) {
                ...
                NavigationLink(destination: LanguageSelectionView(
                    ...
                    HStack {
                        ...
                        Spacer()
                        HStack(spacing: 6) {
                            Text(LanguageHelper.flag(for: settings.sourceLanguageCode))
                            Text(LanguageHelper.languageName(for: settings.sourceLanguageCode))
                ...
                .onChange(of: settings.languageConfiguration.sourceLanguageCode) { oldValue, newValue in
                    ...
                    if newValue == settings.targetLanguageCode {
                ...
                NavigationLink(destination: LanguageSelectionView(
                    ...
                    excludedLanguageCode: settings.sourceLanguageCode
                )) {
                    HStack {
                        Text("Target Language")
                        Spacer()
                        HStack(spacing: 6) {
                            Text(LanguageHelper.flag(for: settings.targetLanguageCode))
                            Text(LanguageHelper.languageName(for: settings.targetLanguageCode))
                                .foregroundStyle(.secondary)
                        }
                    }
                }
                .onChange(of: settings.languageConfiguration.targetLanguageCode) { oldValue, newValue in
                    ...
                    if newValue == settings.sourceLanguageCode {
                        // Find a different language
                        if let differentLang = LanguageHelper.allLanguages.first(where: { $0.code != newValue }) {
